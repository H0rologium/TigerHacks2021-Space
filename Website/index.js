(function (THREE, OrbitControls, d3, topojson, satellite) {
  ...
  function init(parsedTles, topology) {
    ...
    var sphere = new THREE.Mesh(
      new THREE.SphereGeometry(radius - 0.5, 128, 128),
    ...
    satellites = new THREE.Points(
      satGeometry,
      new THREE.PointsMaterial({ color: 0x000000, size: 20 })
    );
    scene.add(satellites);

    // Rotates 90 degrees. Must convert to radians.
    graticule.rotation.x =
      countries.rotation.x =
      satellites.rotation.x =
        (-90 / 180) * Math.PI;
    d3.timer(animate);
  }
